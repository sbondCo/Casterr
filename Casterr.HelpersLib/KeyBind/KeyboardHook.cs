...
namespace Casterr.HelpersLib.KeyBind
{
    public class KeyboardHook
    {
        ...
        // Events
        public event KeyboardHookCallback KeyDown, KeyUp;
        ...
        public void Install()
        {
            ...
            KeyUp += new KeyboardHookCallback(KeyUpEvent);

            Console.WriteLine("hook install");
        }

        private void KeyUpEvent(KeyboardHook.VKeys key)
        {
        ...
        {
            Console.WriteLine("hookfunc");

            if (nCode >= 0)
            {
                Console.WriteLine("hookfunc if");

                int iwParam = wParam.ToInt32();

                if ((iwParam == WM_KEYDOWN || iwParam == WM_SYSKEYDOWN))
                    if (KeyDown != null) KeyDown((VKeys)Marshal.ReadInt32(lParam));
                if ((iwParam == WM_KEYUP || iwParam == WM_SYSKEYUP))
                    if (KeyUp != null) KeyUp((VKeys)Marshal.ReadInt32(lParam));
            }
